...
import { getFakeAmount } from "../../client/src/utils/transactionUtils";

export const userbaseSize = +process.env.SEED_USERBASE_SIZE!;
export const eventsCount = +process.env.SEED_EVENTS_SIZE!;
...
const randomOS = () => {
  ...
  const types: os[] = ["windows", "mac", "linux", "ios", "android", "other"];
  return types[randomNumber];
};
const randomBrowser = () => {
  const randomNumber = Math.floor(Math.random() * 6);
  const types: browser[] = ["chrome", "safari", "edge", "firefox", "ie", "other"];
  return types[randomNumber];
};
const randomUrl = () => {
  const randomNumber = Math.floor(Math.random() * 3);
  const types: string[] = ["login", "home", "admin"];
  return types[randomNumber];
};
...
export const createFakeEvent = (signups: Event[]): Event => {
  ...
  return {
    ...
    name: randomName(),
    distinct_user_id: signups[randomUserIndex].distinct_user_id,
    date: faker.date.between(new Date(), new Date(signups[randomUserIndex].date)).getTime(),
    os: randomOS(),
...
export const buildDatabase = () => {
  ...
  const seedEvents: Event[] = createSeedEvents(seedUsers);
